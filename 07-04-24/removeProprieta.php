...
<head>
    ...
    function createAndPopulateTable(data) {
        ...
        tableHtml += '<tr>';
        tableHtml += '<th>Codice Catastale</th>';
        tableHtml += '<th>Indirizzo</th>';
        tableHtml += '<th>Comune</th>';
        tableHtml += '<th>Prezzo</th>';
        tableHtml += '<th>Descrizione</th>';
        tableHtml += '<th>Tipo</th>';
        tableHtml += '<th>Dimensioni</th>';
        tableHtml += '<th>Note</th>';
        tableHtml += '<th>Azione Proprietà</th>';
        tableHtml += '</tr>';

        data.data.forEach(function (row) {
            tableHtml += '<tr>';
            tableHtml += '<td>' + row.codiceCatastale + '</td>';
            tableHtml += '<td>' + row.indirizzo + '</td>';
            tableHtml += '<td>' + row.comune + '</td>';
            tableHtml += '<td>' + row.prezzo + '</td>';
            tableHtml += '<td>' + row.descrizione + '</td>';
            tableHtml += '<td>' + row.tipo + '</td>';
            tableHtml += '<td>' + row.dimensioni + '</td>';
            tableHtml += '<td>' + row.note + '</td>';
            tableHtml += '<td><button class="remove-button" data-proprietà-id="' + row.codiceCatastale + '">Rimuovi Proprietà</button></td>';
            tableHtml += '</tr>';
        });
        ...
        if (table) {
            table.addEventListener('click', function (e) {
                ...
                if (target && target.classList.contains('remove-button')) {
                    var proprietàId = target.getAttribute('data-proprietà-id');

                    
                    var isConfirmed = confirm('Sei sicuro di voler rimuovere questa proprietà?');
                    if (isConfirmed) {
                        ...
                        xhr.open('GET', 'rimProprieta.php?remove_proprieta=' + proprietàId, true);
                        xhr.send();
                    }
                }
            });
    ...
    function getInitialData() {
        ...
        xhrInitial.open('GET', 'rimProprieta.php', true);
        xhrInitial.send();
    }
