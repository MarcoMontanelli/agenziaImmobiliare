  ...
  <div class="min-h-screen flex flex-col">
    ...
    <?php foreach ($results as $row): ?>
      ...
      <div class="md:w-3/4 p-4 bg-gray-800 flex flex-col justify-between">
        ...
        <button onclick="selectAnnouncement(<?php echo $row['idAnnuncioIr']; ?>);" id="popoverButton<?php echo $row['idAnnuncioIr']; ?>" data-popover-target="popoverContent<?php echo $row['idAnnuncioIr']; ?>" type="button" class="visita-btn text-white relative bg-blue-500 hover:bg-blue-700 font-bold py-2 px-4 rounded">
          VISITA
        </button>


        <button id="addFavoriteBtn" data-announcement-id="<?php echo $row['idAnnuncioIr']; ?>" class="hover:text-gray-400 text-white">
...
<!-- Modal Structure -->
<div id="popup-modal" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5 text-center">
                ...
                <h3 id="modal-message" class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Message goes here</h3>
                <button data-modal-hide="popup-modal" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center" onclick="closeModal()">
                    Close
                </button>
            </div>
        </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to add a favorite using AJAX
    function addFavorite(announcementId) {
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'add_to_favorites.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onload = function() {
            if (this.status === 200) {
                // Assuming your PHP script returns "Success" on success
                if (this.responseText.trim() === 'Success') {
                    showModal('Your favorite has been added successfully!');
                } else {
                    // Handle PHP script response errors (e.g., item already favorited)
                    showModal(this.responseText);
                }
            } else {
                // Handle HTTP errors
                showModal('Failed to add favorite due to a server error. Please try again.');
            }
        };
        ...
        xhr.send('idAnnuncioIr=' + encodeURIComponent(announcementId));
    }
...
<!-- component -->
<!-- Modal Backdrop -->
<div id="calendarModal" class="hidden w-full h-full fixed inset-0 absolute z-[1050] overflow-y-auto bg-black bg-opacity-50">
    <!-- Modal -->
    <div class="flex items-center justify-center min-h-screen">
        <div class="bg-gray-800 shadow-lg rounded-lg overflow-hidden w-11/12 md:w-8/12 lg:w-7/12">
            ...
            <div class="grid grid-cols-7 gap-2 p-4 text-white" id="calendar">
...
async function fetchBookedDates() {
    try {
        const response = await fetch('fetch_booked_dates.php');
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        bookedDates = await response.json();
        console.log(bookedDates); // For debugging
    } catch (error) {
...
function generateCalendar(year, month) {
    ...
    // Create a Date object for each day to use for comparisons and formatting
    const dayDate = new Date(year, month, day);
    // Format the date as "YYYY-MM-DD" to check against bookedDates
    ...
    if (bookedDates.includes(dayString)) {
        ...
        dayElement.classList.remove('cursor-pointer'); // Remove pointer to indicate it's not selectable
        // It's safe to just not assign an onclick handler if the day is booked
    } else {
        // Attach click event for available dates
        dayElement.addEventListener('click', () => {
            // Use dayDate directly for formatting to ensure correctness
            const formattedDate = dayDate.toLocaleDateString('default', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
...
let userId = <?php echo json_encode($_SESSION['user_id'] ?? $_COOKIE['user_id'] ?? ''); ?>;
let selectedAnnouncementId = null; // Global variable to hold the selected announcement ID

function selectAnnouncement(announcementId) {
    selectedAnnouncementId = announcementId;
    console.log("Selected Announcement ID:", selectedAnnouncementId); // For debugging
    openModal(); // Assuming you have this function ready to show the calendar modal
}
...
dayElements.forEach(dayElement => {
    dayElement.addEventListener('click', () => {
        ...
        // Assuming you have a function to handle booking
        bookVisit(dateToSend, selectedAnnouncementId, userId);
    });
});

function bookVisit(date, announcementId, userId) {
    fetch('book_visit.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: `date=${date}&announcementId=${announcementId}&userId=${userId}`
    })
    .then(response => response.json())
    .then(data => {
        console.log(data); // Handle success or error accordingly
    })
    .catch((error) => {
