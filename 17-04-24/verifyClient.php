...
// Redirect if clientId is not present
if (!$clientId) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmtCheckVerification = $conn->prepare("SELECT * FROM verificacl WHERE id_cliente = ?");
    $stmtCheckVerification->bind_param("i", $clientId);
    ...
    if ($row) {
        if ($verificationCode == $row['codiceVerifica']) {
            if ($row['scadenza'] > date('Y-m-d H:i:s')) {
                // Verification successful, update cliente table and delete verification record
                $updateCliente = $conn->prepare("UPDATE cliente SET verificato = 1 WHERE idCliente = ?");
                $updateCliente->bind_param("i", $clientId);
                $updateCliente->execute();

                $stmtDeleteVerification = $conn->prepare("DELETE FROM verificacl WHERE id_cliente = ?");
                $stmtDeleteVerification->bind_param("i", $clientId);
                ...
                $_SESSION['user_id'] = $clientId;
                    ...
                    setcookie('user_id', $clientId, time() + (86400 * 30), "/"); // Set for 30 days
                    setcookie('user_type', 'client', time() + (86400 * 30), "/"); // Set for 30 days
                ...
                // Optionally, handle expired verification code cases, such as deleting records or asking for re-verification
    ...
    <div class="flex flex-wrap overflow-hidden rounded-xl">
      ...
      <div class="w-full md:w-1/2 p-6 flex flex-col justify-center">
        <div class="flex items-center justify-center md:justify-start space-x-3 mb-6">
          ...
          <h2 class="text-lg font-medium text-gray-900">Verifica Cliente</h2>
        </div>
