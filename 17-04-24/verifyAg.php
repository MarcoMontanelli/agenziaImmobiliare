...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmtCheckVerification = $conn->prepare("SELECT * FROM verificaAg WHERE id_agenzia = ?");
    $stmtCheckVerification->bind_param("i", $agencyId);
    ...
    if($row){
        if ($verificationCode == $row['codiceVerifica'] ){
            if($row['scadenza'] > date('Y-m-d H:i:s')){
                ...
                $stmtDeleteVerification = $conn->prepare("DELETE FROM verificaAg WHERE id_agenzia = ?");
                $stmtDeleteVerification->bind_param("i", $agencyId);
                $stmtDeleteVerification->execute();
                $updateVerification = $conn->prepare("UPDATE agenzia_immobiliare SET verificato = 1 WHERE idAgenzia = ?");
                $_SESSION['user_id'] = $agencyId;
                $_SESSION['user_type'] = 'agency';

                // Set cookies for user ID and type
                setcookie('user_id', $agencyId, time() + (86400 * 30), "/"); // Set for 30 days
                setcookie('user_type', 'agency', time() + (86400 * 30), "/"); // Set for 30 days
                echo "Verification successful! Redirecting to home...";
                ...
                $stmtDeleteVerification->bind_param("i", $agencyId);
                $stmtDeleteVerification->execute();
                $stmtDeleteAgenziaImmobiliare = $conn->prepare("DELETE FROM agenzia_immobiliare WHERE idAgenzia = ?");
                $stmtDeleteAgenziaImmobiliare->bind_param("i", $agencyId);
                $stmtDeleteAgenziaImmobiliare->execute();
    ...
    <div class="flex flex-wrap overflow-hidden rounded-xl">
      ...
      <div class="w-full md:w-1/2 p-6 flex flex-col justify-center">
        <div class="flex items-center justify-center md:justify-start space-x-3 mb-6">
          ...
          <h2 class="text-lg font-medium text-gray-900">Verifica Agenzia Immobiliare</h2>
        </div>
