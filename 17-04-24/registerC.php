...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $verificato = 0;
    $stmt = $conn->prepare("INSERT INTO cliente (nome, ragioneSociale, numeroTelefono, email, pass, verificato) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $nome, $ragioneSociale, $numeroTelefono, $email, $password, $verificato);

    if ($stmt->execute()) {
        // Success, now insert into verificacl
        $clienteId = $stmt->insert_id; // Get the last inserted id
        $codiceVerifica = substr(md5(uniqid(mt_rand(), true)), 0, 6);
        $verificato = 0; // Assuming this is a flag that needs to be set initially

        $stmtVerifica = $conn->prepare("INSERT INTO verificacl (id_cliente, codiceVerifica, verificato, scadenza) VALUES (?, ?, ?, NOW() + INTERVAL 3600 SECOND)");
        $stmtVerifica->bind_param("isi", $clienteId, $codiceVerifica, $verificato);
        if($stmtVerifica->execute()) {
            // Verification record inserted successfully
            header("Location: verifyClient.php?clientId=$clienteId");
            exit();
        } else {
            echo json_encode(['error' => "Errore durante la registrazione nella tabella di verifica: " . $stmtVerifica->error]);
        }
    } else {
        echo json_encode(['error' => "Errore durante la registrazione del cliente: " . $stmt->error]);
    }
...
<body class=" bg-no-repeat bg-cover bg-center" style="background-image: url('background.svg')">
    ...
    <div class="min-h-screen flex items-center justify-center bg-no-repeat bg-cover bg-center" style="background-image: url('background.svg');">
    <div class="container max-w-md mx-auto shadow-2xl bg-white rounded-xl p-6">
        <form action="registerC.php" method="post" class="space-y-6">
            <h2 class="text-2xl font-semibold text-gray-900">Registrazione Cliente</h2>
            ...
            <div>
                ...
                <input type="text" id="ragioneSociale" name="ragioneSociale" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            </div>

            <div>
                <label for="numeroTelefono" class="block text-sm font-medium text-gray-700">Numero di Telefono:</label>
                <input type="text" id="numeroTelefono" name="numeroTelefono" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            </div>
            ...
            <div>
                ...
                <input type="password" id="pass" name="pass" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            </div>

            <input type="submit" value="Registrati" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
        </form>
        <p class="mt-6 text-sm text-center text-gray-600">Sei già registrato? <a href="login.php" class="text-blue-600 hover:underline">Accedi</a></p>
        <div class="flex justify-center mt-4">
            <button onclick="location.href='debugPage.php'" class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Torna alla home</button>
        </div>
    </div>
</div>
    </section>
