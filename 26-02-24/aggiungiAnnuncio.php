              ...
              <div class="mb-6">
                ...
                <div class="flex items-center justify-center w-full">
                  <label for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                      ...
                      <input  id="dropzone-file" name="immagini[]" type="file" class="hidden" multiple onchange="handleFiles(this.files)" />
                  </label>
              </div> 
              </div>
...
function updateGallery() {
  ...
  fileArray.forEach((file, index) => {
    ...
    img.onload = () => URL.revokeObjectURL(img.src); // Free memory when image loads

    const button = document.createElement('button');
    button.className = 'absolute top-0 right-0 bg-red-600 rounded-full p-1 m-1';
    button.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>`;
    button.onclick = () => {
      fileArray = fileArray.filter((_, fileIndex) => fileIndex !== index);
      updateGallery();
    };
...
function handleFiles(files) {
  // Add new files to the array, filtering out duplicates
  Array.from(files).forEach(file => {
    if (file.type.startsWith('image/') && !fileArray.includes(file)) {
      fileArray.push(file);
    }
  });
  ...
  // Attempt to clear the input required to prevent duplicates
...
document.getElementById('file-input').addEventListener('change', function() {
  ...
  <script>
    document.getElementById('pubblicaAnnuncio').addEventListener('submit', function(event) {
  ...
  if (!latitude || !longitude) {
    ...
    errorMessage.textContent = 'inserisci un pin sulla mappa prima di continuare.'; // Set error message
    errorMessage.classList.remove('hidden'); 

  } else {
    errorMessage.classList.add('hidden'); // Hide error message if fields are not empty
  }
});
    </script>
