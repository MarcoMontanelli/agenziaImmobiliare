...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $verificato = 0; // Assuming you have a 'verificato' column

    $stmt = $conn->prepare("INSERT INTO ADMIN (nome, località, tipo, email, pass, verificato) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssi", $nome, $locazione, $tipo, $email, $password, $verificato);

    if ($stmt->execute()) {
        $adminId = $stmt->insert_id; // Get the last inserted id
        $codiceVerifica = substr(md5(uniqid(mt_rand(), true)), 0, 6);
        $scadenza = date('Y-m-d H:i:s', strtotime('+1 hour')); // Set expiry time for verification code

        $stmtVerifica = $conn->prepare("INSERT INTO verificaad (id_admin, codiceVerifica, verificato, scadenza) VALUES (?, ?, ?, ?)");
        $stmtVerifica->bind_param("isis", $adminId, $codiceVerifica, $verificato, $scadenza);
        
        if ($stmtVerifica->execute()) {
            header("Location: verifyad.php?adminId=$adminId"); // Redirect to verification page
            exit();
        } else {
            echo "Errore durante la creazione del codice di verifica.";
        }
    } else {
        echo "Errore durante la registrazione dell'admin: " . $stmt->error;
    }
...
<body class=" bg-no-repeat bg-cover bg-center" style="background-image: url('background.svg')">
    ...
    <div class="container max-w-md mx-auto shadow-2xl bg-white rounded-xl p-6">
        <form action="registerA.php" method="post" class="space-y-6">
            <h2 class="text-2xl font-semibold text-gray-900">Registrazione Admin</h2>
            
            <div>
                <label for="nome" class="block text-sm font-medium text-gray-700">Nome Admin:</label>
            ...
            <div>
                <label for="locazione" class="block text-sm font-medium text-gray-700">Locazione:</label>
                <input type="text" id="locazione" name="locazione" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            </div>

            <div>
                <label for="tipo" class="block text-sm font-medium text-gray-700">Tipo:</label>
                <input type="text" id="tipo" name="tipo" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            </div>
